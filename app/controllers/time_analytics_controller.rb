class TimeAnalyticsController < ApplicationController
  ...
  def team_dashboard
    ...
    if @view_mode == 'activity'
      ...
      if @grouping == 'daily'
        ...
        @time_entries.each do |entry|
          ...
          member_data[member_name] ||= 0
          member_data[member_name] += entry.hours
        end
        sorted_data = member_data.sort_by { |member_name, _| member_name }
        @paginated_entries = sorted_data.slice(@offset, @limit).map do |member_name, hours|
          Struct.new(:period, :hours).new(member_name, hours)
        end
      end
    ...
    if @view_mode == 'activity' && ['weekly', 'monthly', 'yearly'].include?(@grouping) && defined?(@activity_pivot_data)
      ...
      @chart_data = generate_project_pivot_chart_data(@project_pivot_data, chart_type, @project_view_state)
    elsif @view_mode == 'members' && ['weekly', 'monthly', 'yearly'].include?(@grouping) && defined?(@member_pivot_data)
